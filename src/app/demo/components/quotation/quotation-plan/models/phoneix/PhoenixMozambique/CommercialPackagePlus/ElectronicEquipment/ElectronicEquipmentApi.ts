...
export class ElectronicEquipmentCommercialApiMozambique {
      customerDetails: any;
    commonDetails: any[] = [];
   ...
   getEditDetails(subDetails, obj) {
      ...
      if(sectionList2.length != 0){
         for (let ele of sectionList2) {
                  console.log('jhsakjak',ele, sectionList2);
                    let houseData =  {
                            SumInsured: ele.SumInsured,
                            Description: ele.DescriptionOfRisk
                    };
    ...
    if (list) {
      ...
      for (let index = 0; index < list.length; index++) {
        if (list[index]?.SumInsured != null && list[index]?.SumInsured != 0 && list[index]?.SumInsured != '0') {
          ...
          if (subEntry['CategoryId']) { subEntry["CategoryDesc"] = industryTypeList.find(ele => ele.Code == subEntry['CategoryId'])?.CodeDesc }
    ...
    let EElist = entry.ElectronicPortableEquipmentList;
    if (EElist) {
      console.log(EElist);
