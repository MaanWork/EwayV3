...
export class HouseHoldersContentsPersonalApiMozambique{
    customerDetails: any;
    commonDetails: any[]=[];
    ...
    extendsfields:FormlyFieldConfig
      ...
      getSaveDetails(entry,constructionList,IndustryId,industryTypeList,obj,list){
            for(let i =0; i < list.length; i++){
                if(list[i].ContentInsured!='' && list[i].ContentInsured!=undefined && list[i].ContentInsured!='0' && list[i].ContentInsured!='undefined'){
                  let subEntry= {
                    "SectionId": "228",
