...
export class PublicLiabilityApiPhoenix{
    customerDetails: any;
    commonDetails: any[]=[];
  ...
  extendsfields:FormlyFieldConfig
    ...
    getEditDetails(subDetails,obj){
      ...
      if (PublicLiabilityPhoenix.length != 0) {
        let GeneralLiabilityList = PublicLiabilityPhoenix.filter(ele => ele.CoverId == '435' || ele.CoverId == 435 );
        if (GeneralLiabilityList.length != 0) { obj['GeneralLiability'] = GeneralLiabilityList[0].CategoryId; }
        let LegalDefenceCostsList = PublicLiabilityPhoenix.filter(ele => ele.CoverId == '438' || ele.CoverId == 438);
        if (LegalDefenceCostsList.length != 0) { obj['LegalDefenceCosts'] = LegalDefenceCostsList[0].CategoryId; }
        let WrongfulArrestandDefamationList = PublicLiabilityPhoenix.filter(ele => ele.CoverId == '439' || ele.CoverId == 439);
        if (WrongfulArrestandDefamationList.length != 0) { obj['WrongfulArrestandDefamation'] = WrongfulArrestandDefamationList[0].CategoryId; }
        let SpreadofFireList = PublicLiabilityPhoenix.filter(ele => ele.CoverId == '440' || ele.CoverId == 440);
        if (SpreadofFireList.length != 0) { obj['SpreadofFire'] = SpreadofFireList[0].SumInsured; }
        let FoodandDrinkList = PublicLiabilityPhoenix.filter(ele => ele.CoverId == '441' || ele.CoverId == 441);
        if (FoodandDrinkList.length != 0) { obj['FoodandDrink'] = FoodandDrinkList[0].SumInsured; }
        let ForecourtServiceStationExtensionList = PublicLiabilityPhoenix.filter(ele => ele.CoverId == '442' || ele.CoverId == 442);
        ...
        let CarWashandValetExtensionList = PublicLiabilityPhoenix.filter(ele => ele.CoverId == '443' || ele.CoverId == 443);
        if (CarWashandValetExtensionList.length != 0) { obj['CarWashandValetExtension'] = CarWashandValetExtensionList[0].SumInsured; }
        let AdditionalclaimsPreparationCostsList = PublicLiabilityPhoenix.filter(ele => ele.CoverId == '372' || ele.CoverId == 372);
        ...
        let ProductsLiabilityList = PublicLiabilityPhoenix.filter(ele => ele.CoverId == '436' || ele.CoverId == 436);
        ...
        let DefectiveWorkmanshipList = PublicLiabilityPhoenix.filter(ele => ele.CoverId == '437' || ele.CoverId == 437);
