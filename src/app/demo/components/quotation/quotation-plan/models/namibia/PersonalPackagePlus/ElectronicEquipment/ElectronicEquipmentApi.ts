...
export class ElectronicEquipmentPersonalApiNamibia {
  customerDetails: any;
  commonDetails: any[] = [];
  ...
  getEditDetails(subDetails, obj) {
    ...
    if (ElectronicEquipSection.length != 0) {
      obj['ElectronicEquipment'] = [];
      obj['ElectronicPortableEquipment'] = [];
      ...
      obj['filled'] = ElectronicEquipSection.length > 0 ? true : false
      let sectionList = ElectronicEquipSection.filter(ele => ele.CoverId == 90 || ele.CoverId == '90');
      if (sectionList.length != 0) {
        for (let ele of sectionList) {
          ...
          obj['ElectronicEquipment'].push(houseData);
        }
      }
      let sectionList2 = ElectronicEquipSection.filter(ele => ele.CoverId == 501 || ele.CoverId == '501');
      if (sectionList2.length != 0) {
        for (let ele of sectionList2) {
          let houseData = {
            SumInsured: ele.SumInsured,
            Description: ele.DescriptionOfRisk
          };
          obj['ElectronicPortableEquipment'].push(houseData);
        }
      }
      return obj
    }
  }
  getSaveDetails(entry, industryTypeList, claimCostList, obj, objAlt) {
    let list = entry.ElectronicEquipment;
    if (list) {
      console.log(list);
      for (let index = 0; index < list.length; index++) {
        if (list[index]?.SumInsured != null && list[index]?.SumInsured != 0 && list[index]?.SumInsured != '0') {
          let subEntry = {
            "SectionId": "76",
            "SectionName": "Electronic Equipment",
            "CoverId": "90",
            "SumInsured": String(list[index]?.SumInsured).replaceAll(',', ''),
            "CategoryId": list[index]?.CategoryId,
            "DescriptionOfRisk": list[index]?.Description
          }
          ...
          if (subEntry['CategoryId']) { subEntry["CategoryDesc"] = claimCostList.find(ele => ele.Code == subEntry['CategoryId'])?.CodeDesc }
          obj.SectionList.push(subEntry);
        }
      }
    }
    let EElist = entry.ElectronicPortableEquipment;
    if (EElist) {
      console.log("Port List", EElist);
      for (let index = 0; index < EElist.length; index++) {
        if (EElist[index]?.SumInsured != null && EElist[index]?.SumInsured != 0 && EElist[index]?.SumInsured != '0') {
          let subEntry = {
            "SectionId": "76",
            "SectionName": "Electronic Equipment",
            "CoverId": "501",
            "SumInsured": String(EElist[index]?.SumInsured).replaceAll(',', ''),
            "CategoryId": EElist[index]?.CategoryId,
            "DescriptionOfRisk": EElist[index]?.Description
          }
          if (entry['IndustryType']) { subEntry['IndustryType'] = entry['IndustryType']; subEntry["IndustryTypeDesc"] = industryTypeList.find(ele => ele.Code == entry['IndustryType'])?.CodeDesc }
          if (subEntry['CategoryId']) { subEntry["CategoryDesc"] = claimCostList.find(ele => ele.Code == subEntry['CategoryId'])?.CodeDesc }
          obj.SectionList.push(subEntry);
