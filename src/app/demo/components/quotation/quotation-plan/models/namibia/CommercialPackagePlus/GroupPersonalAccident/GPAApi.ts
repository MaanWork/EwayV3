...
export class GPACommercialApiNamibia {
  customerDetails: any;
  commonDetails: any[] = [];
    ...
    GPASection.forEach(item => {
      ...
      if (existingEntry) {
        ...
        obj.list.push({
          ...
          "IndustryTypeDesc": item.IndustryTypeDesc
        });
      }
    });
console.log("After Grouping",obj)
   obj['filled'] = GPASection.length > 0 ? true: false 
   return obj;
  }
  getSaveDetails(entry, constructionList, IndustryId, industryTypeList, obj, list) {
     console.log("Before Grouping",obj)
