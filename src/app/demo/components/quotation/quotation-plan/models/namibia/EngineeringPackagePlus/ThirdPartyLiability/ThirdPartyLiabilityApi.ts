  ...
  extendsfields:FormlyFieldConfig
    ...
    getEditDetails(subDetails,obj){
        ...
        if(TPLSection.length!=0){
          ...
          if(Spreadoffire.length!=0){obj['Spreadoffire']=Spreadoffire[0].SumInsured;obj['SpreadoffireDesc']=Spreadoffire[0].DescriptionOfRisk;obj['IndustryType']=null;if(Spreadoffire[0]?.IndustryType!='0')obj['IndustryType']=Spreadoffire[0].IndustryType;}
          let ThirdPartyLiability = TPLSection.filter(ele=>ele.CoverId==460 || ele.CoverId=='460')
          if(ThirdPartyLiability.length!=0){obj['ThirdPartyLiability']=ThirdPartyLiability[0].SumInsured;obj['ThirdPartyLiabilityDesc']=String(ThirdPartyLiability[0].CategoryId);obj['IndustryType']=null;if(ThirdPartyLiability[0]?.IndustryType!='0')obj['IndustryType']=ThirdPartyLiability[0].IndustryType;}
          ...
          obj['filled'] = TPLSection.length > 0 ? true : false
        return obj
      }
    }
    getSaveDetails(entry,claimCostList,IndustryId,industryTypeList,obj,liabilityList){
      console.log(entry);
        ...
        if(entry.Spreadoffire!='0' && entry.Spreadoffire!=null && entry.Spreadoffire!=''){
            let subEntry= {
              ...
              "DescriptionOfRisk":entry.SpreadoffireDesc
            }
          ...
          if(entry.ThirdPartyLiability!='0' && entry.ThirdPartyLiability!=null && entry.ThirdPartyLiability!='' && entry.ThirdPartyLiabilityDesc!=null && entry.ThirdPartyLiabilityDesc!=''){
            let subEntry= {
              "SectionId": "185",
              "SectionName":"THIRD PARTY LIABILITY",
              "CoverId":"460",
              "SumInsured": entry.ThirdPartyLiability,
              "CategoryId":entry.ThirdPartyLiabilityDesc,
              "CategoryDesc":liabilityList.find(ele=>ele.Code==entry.ThirdPartyLiabilityDesc)?.CodeDesc?.replaceAll(',',''),
            }
            if (IndustryId) { subEntry['IndustryType'] = IndustryId; subEntry["IndustryTypeDesc"] = industryTypeList.find(ele => ele.Code == IndustryId)?.CodeDesc }
            obj.SectionList.push(subEntry);
          }
