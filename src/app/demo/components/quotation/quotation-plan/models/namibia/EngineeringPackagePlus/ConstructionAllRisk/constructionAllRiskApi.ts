...
export class ConstructionAllRiskEngineeringApiPhoenix{
    customerDetails: any;
    commonDetails: any[]=[];
    ...
    getEditDetails(subDetails,obj,info){
        let CARSection  = subDetails.filter(ele=>ele['SectionId']=='229');
          let z;
        for (let x = 0; x < info.length; x++) {
          if(info[x].SectionId == 229) z=x;
        }
                            ...
                            if(CARSection.length!=0){
                              ...
                              if(info){ obj['CARDescription']=info[z]?.Description;}
                              if(info){ obj['CARAnnual']=info[z]?.AnnualOpen;}
                              if(info){ obj['CARPrincipal']=info[z]?.PrincipalOwner;}
                              if(info){ obj['CARLocationName']=info[z]?.LocationName;}
                              if(info){ obj['CARStartDate']=info[z]?.StartDate;}
                              if(info){ obj['CARPeriodOfActivity']=info[z]?.PeriodOfActivity;}
                              console.log(obj);
                              obj['filled'] = CARSection.length > 0 ? true : false
                              return obj
                            }
    }
    getSaveDetails(entry,IndustryId,industryTypeList,obj){
          ...
          if(entry.CARInland!='0' && entry.CARInland!=null && entry.CARInland!=''  && entry.CARInland!='undefined' && entry.CARInland!=undefined){
            let subEntry= {
              "SectionId": "229",
              "SectionName":"Construction All Risk Section",
              "CoverId":"552",
