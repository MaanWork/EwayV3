...
export class BusinessAllRiskApiTanzaniya{
    customerDetails: any;
    commonDetails: any[]=[];
  ...
  extendsfields:FormlyFieldConfig
    ...
    getEditDetails(subDetails,obj,type?){
       
        let BussinessAllRiskList = subDetails.filter(ele => ele['SectionId'] == '223');
        if (BussinessAllRiskList.length != 0) {
          if(type){
            let elecEquipment = BussinessAllRiskList.filter(ele => ele.CoverId == 385 || ele.CoverId == '385')
    ...
    getSaveDetails(entry,ClaimCostList,industryTypeList,obj,type?){
        if(type){
