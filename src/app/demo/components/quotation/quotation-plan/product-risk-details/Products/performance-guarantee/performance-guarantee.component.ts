...
import { PerformanceGuranteePPNamibia } from '../../../models/namibia/GuranteePackagePlus/PerformanceGurantee/PerformanceGurantee';
...
@Component({
  selector: 'app-performance-guarantee',
  templateUrl: './performance-guarantee.component.html',
  styleUrls: ['./performance-guarantee.component.css']
})
export class PerformanceGuaranteeComponent implements OnInit {
      ...
      ngOnInit(){
         let fireData = null;
          if(this.productId=='98'){
           ...
           fireData = new PerformanceGuranteePPNamibia();
          } 
          else{
         ...
         if (this.renderType == 'Direct') {
            ...
            for (let obj of this.locationList) {
                if (this.locationDetails[i]) {
                    let performanceGuaranteeApi = null,subDetails=this.locationDetails[i].SectionList;

                         performanceGuaranteeApi = new PerformanceGuranteeApiNamibia();
                    obj = performanceGuaranteeApi.getEditDetails(subDetails, obj, this.engineerData);
      ...
      onProceedData(type) {
        let j = 0,locationList = [];
            for (let entry of this.locationList) {
                ...
                let performanceGuaranteeRisk = null;

                   if (this.insuranceId == '100050')   performanceGuaranteeRisk = new PerformanceGuranteeApiNamibia();
                if (entry['CARPrincipal'] || entry['CARDescription'] || entry['PeriodType'] || entry['CARPeriodOfActivity'] || entry['CARStartDate'] || entry['ProjectSite']) {
                  let constructionAllRiskList = performanceGuaranteeRisk.getSaveDetails(entry, entry.IndustryId, this.industryTypeList, obj)
